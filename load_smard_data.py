#!/usr/bin/env python3
"""
Load the source SMARD data used by the starter model for one target date.
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Load the source SMARD data used by the starter model."
    )
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="loaded_smard_data.json",
        help="Path to the saved source-data JSON (default: loaded_smard_data.json).",
    )
    ...
    try:
        series = fetch_smard_series(context=context, delivery_date=delivery_date)
    except Exception as exc:
        print(f"Error: {exc}", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Saved SMARD source data to {output_path.resolve()}")
